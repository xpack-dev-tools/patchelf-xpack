...
# How to install the xPack NixOS PatchELF
...
[![GitHub Release Date](https://img.shields.io/github/release-date/xpack-dev-tools/patchelf-xpack)](https://github.com/xpack-dev-tools/patchelf-xpack/releases)
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fpatchelf?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/patchelf/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/patchelf-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fpatchelf?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/patchelf/)

The **xPack NixOS PatchELF** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install NixOS PatchELF
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/patchelf`}><code>@xpack-dev-tools/patchelf</code></a> from
...
<code>xpacks/@xpack-dev-tools/patchelf</code>.
...
**patchelf** package into the project.
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/patchelf@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of NixOS PatchELF, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/patchelf</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/patchelf/.content/bin</code>;
* add <code>@xpack-dev-tools/patchelf</code> to
  `package.json` as a
  development dependency; this associates a specific version of NixOS PatchELF with
...
it is also possible to install NixOS PatchELF only globally in
...
In order to access the NixOS PatchELF binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/patchelf/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/patchelf/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
the <a href={`https://github.com/xpack-dev-tools/patchelf-xpack/releases`}>GitHub Releases</a> pages.
...
<ul>
<li><code>xpack-patchelf-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-patchelf-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
To manually install the <b>xPack NixOS PatchELF</b>,
unpack the archive and move it to a location of your choice.

The recommended location is
the <code>~/Library/xPacks/patchelf</code> folder, for
example <code>/Users/ilg/Library/xPacks/patchelf/xpack-patchelf-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/patchelf
cd ~/Library/xPacks/patchelf

tar xvf ~/Downloads/xpack-patchelf-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-patchelf-${customField('xpackVersion')}
`} </CodeBlock>
...
<ul>
<li><code>xpack-patchelf-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-patchelf-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-patchelf-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/.local/xPacks/patchelf</code> folder, for
example <code>/home/ilg/.local/xPacks/patchelf/xpack-patchelf-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/patchelf
cd ~/.local/xPacks/patchelf

tar xvf ~/Downloads/xpack-patchelf-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-patchelf-${customField('xpackVersion')}
`} </CodeBlock>
