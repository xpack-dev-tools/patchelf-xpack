...
The easiest (and recommended) way to install xPack NixOS PatchELF
...
* add <code>@xpack-dev-tools/patchelf</code> to
  `package.json` as a
  development dependency; this associates a specific version of xPack NixOS PatchELF with
...
it is also possible to install xPack NixOS PatchELF only globally in
...
In order to access the xPack NixOS PatchELF binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/patchelf/0.18.0-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/patchelf/0.18.0-1.1/.content/bin:$PATH
`} </CodeBlock>
...
<ul>
<li><code>xpack-patchelf-0.18.0-1-darwin-x64.tar.gz</code></li>
<li><code>xpack-patchelf-0.18.0-1-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/patchelf/xpack-patchelf-0.18.0-1</code>:
...
<ul>
<li><code>xpack-patchelf-0.18.0-1-linux-x64.tar.gz</code></li>
<li><code>xpack-patchelf-0.18.0-1-linux-arm64.tar.gz</code></li>
<li><code>xpack-patchelf-0.18.0-1-linux-arm.tar.gz</code></li>
</ul>
...
example <code>/home/ilg/.local/xPacks/patchelf/xpack-patchelf-0.18.0-1</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/patchelf
cd ~/.local/xPacks/patchelf

tar xvf ~/Downloads/xpack-patchelf-0.18.0-1-linux-x64.tar.gz
chmod -R -w xpack-patchelf-0.18.0-1
`} </CodeBlock>
