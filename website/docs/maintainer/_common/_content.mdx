...
**@xpack-dev-tools/patchelf** is an xpm package that packages platform-specific archives, built with the [xPack Build Box (XBB)](https://xpack.github.io/xpack-build-box/).
...
```sh
rm -rf ~/Work/xpack-dev-tools/patchelf-xpack.git/package-lock.json
```
...
npm --prefix ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets install
xpm install -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
...
(such as **0.18.0-1.1**) with the following expression:
...
xpm run trigger-workflow-test-docker-linux-arm64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
- In `CHANGELOG.md`, add the release date and a message
  such as _**\* v0.18.0-1 released**_
...
tagged such as **v0.18.0-1** (mind the dash in the middle!) and
named such as **xPack NixOS PatchELF v0.18.0-1** (mind the dash),
...
- Navigate to the [GitHub Releases](https://github.com/xpack-dev-tools/patchelf-xpack/releases) page
...
npm --prefix ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets install && \
xpm install -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets && \
xpm run test-release -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
