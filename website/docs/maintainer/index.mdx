...
- determine the version (like <code>0.18.0-1.1</code>)
- update the version in top `package.json`
- use the new version, suffixed by `.pre`,
like <code>0.18.0-1.1.pre</code>.
...
Determine the version (like <code>0.18.0-1</code>)
and update the `build-assets/scripts/VERSION` file; the format
is <code>0.18.0-1</code>.
...
- https://github.com/xpack-dev-tools/patchelf-xpack/pulls

and fix them; assign them to a milestone (like <code>0.18.0-1</code>).
...
- add a new entry like _**\* v0.18.0-1 prepared**_
- commit with the message _**prepare v0.18.0-1**_
...
```sh
rm -f ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets/package-lock.json
git -C ~/Work/xpack-dev-tools/patchelf-xpack.git pull

xpm run install -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets

git -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git pull
xpm link -C ~/Work/xpack-dev-tools/xbb-helper-xpack.git

xpm run link-deps -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets

xpm run deep-clean --config darwin-x64  -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets && \
xpm install --config darwin-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets && \
xpm run build-development --config darwin-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```

For a debug build:

```sh
xpm run build-development-debug --config darwin-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
(like **0.18.0**) with the following expression:
...
```sh
xpm run check-space -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
```sh
xpm run clear-all-projects-builds -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
```sh
xpm run generate-workflows -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-build-darwin-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-build-darwin-arm64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-build-linux-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-build-linux-arm64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-build-linux-arm -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
[GitHub Actions](https://github.com/xpack-dev-tools/patchelf-xpack/actions) page.
...
```sh
xpm run trigger-workflow-test-prime -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-x64 -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run trigger-workflow-test-docker-linux-arm -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
```sh
xpm run trigger-travis-macos -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
git -C ~/Work/xpack-dev-tools/patchelf-xpack.git pull
xpm run install -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
xpm run test-pre-release -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
```sh
xpm run trigger-workflow-publish-release -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
watching the patchelf-xpack project.
...
```sh
xpm run generate-website-blog-post -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```

- edit the front matter properties
- select the correct summary
- commit with the message _**website: blog post release 0.18.0-1 published**_
...
```sh
xpm run update-package-binaries -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
- commit all changes with the message
  _**package.json: update urls for 0.18.0-1.1 release**_

### Publish on the npmjs.com server

- select the `xpack-development` branch
- check the latest commits `xpm run git-log`
- update `CHANGELOG.md`, add a line like _**\* v0.18.0-1.1 published on npmjs.com**_
- commit with the message _**CHANGELOG: publish npm v0.18.0-1.1**_
...
<a href="https://www.npmjs.com/package/@xpack-dev-tools/patchelf?activeTab=versions">npmjs.com</a>.
...
```sh
xpm run trigger-workflow-test-xpm -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
in https://xpack-dev-tools.github.io/patchelf-xpack/blog/
- remember the post URL, since it must be used to update the release page
and the X/Twitter announcement
- switch to https://xpack-dev-tools.github.io/patchelf-xpack/docs/maintainer/#update-the-xpack-branch
...
```sh
xpm run trigger-workflow-deep-clean -C ~/Work/xpack-dev-tools/patchelf-xpack.git/build-assets
```
...
- paste the release name like _**xPack NixOS PatchELF v0.18.0-1 released**_
...
[![npm (scoped)](https://img.shields.io/npm/v/@xpack-dev-tools/patchelf)](https://www.npmjs.com/package/@xpack-dev-tools/patchelf/)
[![npm](https://img.shields.io/npm/dm/@xpack-dev-tools/patchelf)](https://www.npmjs.com/package/@xpack-dev-tools/patchelf/)
  - all releases [![npm](https://img.shields.io/npm/dt/@xpack-dev-tools/patchelf)](https://www.npmjs.com/package/@xpack-dev-tools/patchelf/)
